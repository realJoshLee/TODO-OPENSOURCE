      ...
      <div class="row">
        ...
        <div id="settings" class="column">
          ...
          <div class="folders">
            ...
            <a href="work.php" class="folder-link-button"><img src="assets/images/work-active.svg" id="folder-icon" class="work-button"></a>
        ...
        <div id="task-lists" class="column">
          ...
          <?php if(!empty($items)): ?>
          <ul class="items">
            <?php foreach($items as $item): ?><?php if (!$item['done']):?><li><a href="functions.php?as=workdone&item=<?php echo $item['id'] ?>" class="done-button"><span class="dot"></span></a>&nbsp;<div class="task-dropdown"><!--Button to show more in the task dropdown--><button class="task-dropbtn"><!--More image--><img src="assets/images/more-707070.svg" class="more"></button><div class="task-dropdown-content"><!--Inbox move--><a href="functions.php?as=worktoinbox&item=<?php echo $item['id'] ?>" class="done-button"><img src="assets/images/inbox-active.svg" id="bookmark" height="15px" width="15px"></a><!--Personal move--><a href="functions.php?as=worktopersonal&item=<?php echo $item['id'] ?>" class="done-button"><img src="assets/images/user-active.svg" id="bookmark" height="15px" width="15px"></a><!--Bookmark--><a href="functions.php?as=workbookmark&item=<?php echo $item['id'] ?>" class="done-button"><img src="assets/images/bookmark-active.svg" id="bookmark" height="15px" width="15px"></a><!--Delete task--><a href="functions.php?as=workdelete&item=<?php echo $item['id'] ?>" class="done-button"><img src="assets/images/trash-warn.svg" id="bookmark" height="15px" width="15px"></a></div></div>&nbsp;<span class="item<?php echo $item['done'] ? 'done' : '' ?>"><form id="task-form" method="post" action="task-update.php"><input onchange="this.form.submit()" name="tasktext" type="text" class="task-box" value="<?php echo $item['name']; ?>"><input class="display-none" type="text" name="taskid" value="<?php echo $item['id']; ?>"></form></span></li><?php endif; ?><?php endforeach; ?>
          </ul>
          <?php else: ?>
            <!--What is shown when there aren't any items in the list-->
            <p>You haven't added any work tasks yet. Add some below to get started.</p>
          <?php endif; ?>

          <!--The form where you add a task-->
          <form class="item-add" action="work-add.php" method="post">
