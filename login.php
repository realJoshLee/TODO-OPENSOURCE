  ...
  if (empty($_POST["username"]) || empty($_POST["password"])) {
    ...
    if($whitelistacti=='true'){
      if(in_array( $domain, $whitelist ) ) { 
        ...
        $query = "INSERT INTO passwordlogin(accountid, firstname, lastname, username, password, recovery, sessionid, preminum, token) VALUES('$accountid', '$first','$last','$username', '$password', '$recovery', '$session','$preminum','$token')";
      ...
      while ($row = mysqli_fetch_array($result)) {
        ...
        if($row['status']=='active'){   
          if(password_verify($password, $row["password"])) {
            // When the right credentials are entered
          ...
          echo '<script>alert("User account suspended")</script>';
        }
