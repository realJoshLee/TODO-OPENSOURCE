  ...
  <span class="rownav" id="hovereffect">
    ...
    <span class="columnnav textbox">
      ...
      <div class="dropdown dropdown-<?php echo $item['id']; ?>" data-id="<?php echo $item['id']; ?>">
        ...
        <!--Grey-->
        <a href="actions.php?as=default&item=<?php echo $item['id']; ?>" class="navlink"><span style="height: 16px; width: 16px; display: inline-block; border: 2px solid grey; border-radius: 6px; font-size: 10px; background-color: transparent;" class="checkboxcolor"></span></a>

        &nbsp;

        <!--Red-->
        <a href="actions.php?as=red&item=<?php echo $item['id']; ?>" class="navlink"><span style="height: 16px; width: 16px; display: inline-block; border: 2px solid #ff006a; border-radius: 6px; font-size: 10px; background-color: #fccce1;" class="checkboxcolor"></span></a>

        &nbsp;

        <!--Green-->
        <a href="actions.php?as=green&item=<?php echo $item['id']; ?>" class="navlink"><span style="height: 16px; width: 16px; display: inline-block; border: 2px solid #00ff26; border-radius: 6px; font-size: 10px; background-color: #dbffe1;" class="checkboxcolor"></span></a>

        &nbsp;

        <!--Blue-->
        <a href="actions.php?as=blue&item=<?php echo $item['id']; ?>" class="navlink"><span style="height: 16px; width: 16px; display: inline-block; border: 2px solid #006fff; border-radius: 6px; font-size: 10px; background-color: #cfe4ff;" class="checkboxcolor"></span></a>

        &nbsp;

        <!--Orange-->
        <a href="actions.php?as=orange&item=<?php echo $item['id']; ?>" class="navlink"><span style="height: 16px; width: 16px; display: inline-block; border: 2px solid #ff5900; border-radius: 6px; font-size: 10px; background-color: #ffe3d4;" class="checkboxcolor"></span></a>

        <!--<br><br>

        <form method="post" action="notes_update_nav.php?item=<?php echo $item['id']; ?>">
          <?php $decryptednotes = openssl_decrypt(base64_decode($item['notes']), $method, $key, OPENSSL_RAW_DATA, $iv);?>
          <textarea style="width: 100%; height: 75px; outline: none; background: transparent; border: transparent;" class="form-control-notes" name="notes" placeholder="Start typing to add notes..."><?php echo $decryptednotes; ?></textarea><br>
          <input style="color: #006fff; background: transparent; border: transparent; outline: none;" type="submit" class="dropdownsubmit" value="Update Notes">
        </form>-->
